...
const Profile = () => {
  ...
  const [img, setImg] = useState(""); // we need this becoz we need to upload the image in firebase and that will be done when the image will be available
  ...
  return user ? (
    <div className="flex  h-64  items-center justify-center">
      {console.log("hello deelte hua kya")}
      {/*profile container */}
      <div className="flex space-x-4 items-center rounded-md border-2 p-5  w-5/12  shadow-xl">
        <div className="realtive p-3 ">
          <img
            src={user.avatar ? user.avatar : Img} // ye user.avatar se usse url milra hai jiski vjh se mila src and so its showing
            alt="avatar"
          ...
          />
          <div className="absolute top-52 left-1/3 items-center  p-3">
            <label htmlFor="photo">
              <button onClick={handleOpenFileInput}>
                <AiFillEdit className="w-5 h-5 opacity-70 hover:opacity-90" />
              </button>
              <button onClick={deleteImage} className="bottom-0">
                {user.avatar ? (
                  <AiOutlineDelete className=" w-5 h-5 opacity-70 hover:opacity-90" />
                ) : null}
              </button>
            </label>
